...
class XGBoost(object):
    ...
    def fit(self, T = 10, min_train_err = 1e-6, tree_parameters: dict = {}, base = "RegReee"):
        if base == "RegTree":
            self._fit_reg(T, min_train_err, tree_parameters)
        elif base == "DecTree":
            self._fit_dec(T, min_train_err, tree_parameters)
    
    
    def _fit_reg(self, T, min_train_err, tree_parameters):
        m, n = self.y.shape
        self.T = T
        y_t = np.zeros((m, 1))
    ...
    def _fit_dec(self, T, min_train_err, tree_parameters):
        m, n = self.y.shape
        self.T = T
        y_t = np.zeros((m, 1))
